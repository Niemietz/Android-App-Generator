	...
	return (
		<div className="sdk-card">
			<div className="entity-card-head">
				...
				<button className="btn-icon remove-sdk" type="button" title="Remove SDK"
				        onClick={() => actions.removeSdk(sdk.id)}>
					✕
				</button>
			</div>
			...
			<div className="interfaces-container">
				{sdk.interfaces.map((iface, index) => (
					<InterfaceCard sdkIndex={sdkIndex} interfaceIndex={index} key={iface.id} sdkId={sdk.id}
					               iface={iface} actions={actions}/>
				))}
			</div>
			<button className="btn-ghost small add-interface" type="button"
			        onClick={() => actions.addInterface(sdk.id)}>
				+ Add interface
			</button>
		</div>
	);
