...
function makeEntity(name) {
  return {
    ...
    fields: [makeField('title', 'String'), makeField('isDone', 'Boolean')],
  };
}
...
export default function useGeneratorState() {
  ...
  const setIncludeFirebase = (checked) =>
    setState((s) => ({
      ...s,
      project: { ...s.project, includeFirebase: checked, includeSqlConnectVariant: checked ? s.project.includeSqlConnectVariant : false, includeFirestore: checked ? s.project.includeFirestore : false },
  ...
  const updateSdk = (id, patch) =>
    setState((s) => ({ ...s, externalSdks: s.externalSdks.map((sdk) => (sdk.id === id ? { ...sdk, ...patch } : sdk)) }));
